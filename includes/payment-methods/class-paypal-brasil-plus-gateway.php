...
class PayPal_Brasil_Plus_Gateway extends PayPal_Brasil_Gateway {
	...
	public function execute_payment( $order, $payment_id, $payer_id ) {
		...
		$this->api->update_payment( $payment_id, $patch_data, array(), 'ec' );
		$execution_response = $this->api->execute_payment( $payment_id, $payer_id, array(), 'ec' );
	...
	public function create_payment_for_order( $data, $order, $dummy = false ) {
		...
		try {
			// Create the payment.
			$result = $this->api->create_payment( $payment_data, array(), 'ec' );
